...
def index():
    ...
    if request.method == 'POST':
        ...
        acc_hash = ssdeep.hash(request.json['mimetypes'].encode('utf-8') + request.json['fonts_all'].encode('utf-8') + request.json['plugins_all'].encode('utf-8'))
        inacc_hash = ssdeep.hash(str(request.json['timezone']) + request.json['os'] + request.json['screen'])
